#!/usr/bin/env bash
# One-time bootstrap for a fresh Ubuntu 22.04 EC2 instance. Idempotent: safe to re-run
# (docker install and repo clone both skip if already present). Identical in spirit to
# deploy/oracle/setup.sh - same image, same steps, different cloud.
set -euo pipefail
...
echo "From outside:       curl -s http://<this-instance-public-ip>/api/health"
